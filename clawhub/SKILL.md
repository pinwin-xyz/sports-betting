...
**CRITICAL — use the bundled script, do not parse raw subgraph JSON manually.**

The subgraph returns one row per outcome (not per game). Parsing it with `jq` or `grep` will produce duplicates and wrong odds. The bundled script handles grouping, main market detection, and dictionary translation correctly.
...
node scripts/get-games.js football premier-league 10
node scripts/get-games.js football 10               # all football
```
...
```json
POST data-feed with where: { "conditionId": "<id>" }
```
