...
const DATAFEED_URL   = 'https://thegraph-1.onchainfeed.org/subgraphs/name/azuro-protocol/azuro-data-feed-polygon'
...
;(async () => {
  ...
  // ── Step 2 (verify): Re-check condition is Active ────────────────────────
  console.log('\n⏳ Verifying condition is still Active...')
  const datafeedHostname = DATAFEED_URL.replace('https://', '').split('/')[0]
  const datafeedPath     = '/' + DATAFEED_URL.replace('https://', '').split('/').slice(1).join('/')

  const checkRes = await postJson(datafeedHostname, datafeedPath, {
    query: `{ conditions(where: { conditionId: "${conditionId}" }) { conditionId state outcomes { outcomeId currentOdds } } }`
  })

  if (checkRes.errors) {
    console.error('❌ Subgraph error:', JSON.stringify(checkRes.errors))
    process.exit(1)
  }

  const condition = checkRes.data && checkRes.data.conditions && checkRes.data.conditions[0]
  if (!condition) {
    console.error('❌ Condition not found in data-feed. It may have been removed or the conditionId is wrong.')
    process.exit(1)
  }
  if (condition.state !== 'Active') {
    console.error(`❌ Condition is no longer Active (state: ${condition.state}). Market has closed.`)
    process.exit(1)
  }

  // Fetch live odds for this outcome
  const liveOutcome = condition.outcomes.find(o => String(o.outcomeId) === String(outcomeId))
  if (!liveOutcome) {
    console.error(`❌ outcomeId ${outcomeId} not found in condition ${conditionId}.`)
    console.error('   Available outcomeIds:', condition.outcomes.map(o => o.outcomeId).join(', '))
    process.exit(1)
  }

  const currentOdds = oddsArg ? parseFloat(oddsArg) : parseFloat(liveOutcome.currentOdds)
